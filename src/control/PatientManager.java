 ...
 */
    ...
    // If specified, add the patient to the active queue
    if (inQueue) {
        patientQueue.enqueue(patient);
    }
    
    // Update the auto-increment counters based on the added patient
    try {
        // PATIENT ID COUNTER UPDATE:
        // Extract numeric portion from patient ID (format "P001")
        String pid = patient.getPatientID().substring(1); // Remove 'P' prefix
        int currentPid = Integer.parseInt(pid);
        
        // Ensure our patientCounter is always higher than any existing ID
        if (currentPid >= patientCounter) {
            patientCounter = currentPid + 1;
        }
        
        // QUEUE ID COUNTER UPDATE (only if patient is being queued):
        if (inQueue && patient.getQueueID() != null) {
            // Extract numeric portion from queue ID (format "Q001")
            String qid = patient.getQueueID().substring(1); // Remove 'Q' prefix
            int currentQid = Integer.parseInt(qid);
            
            // Ensure our queueCounter is always higher than any existing queue ID
            if (currentQid >= queueCounter) {
                queueCounter = currentQid + 1;
            }
        }
    } catch (Exception e) {
        // Log any errors in counter updates without crashing
        System.err.println("Error updating counters: " + e.getMessage());
        // Note: The patient is still added even if counter update fails
    }
}
...
//-----------------------------------------------------------------------------------------------------------------//    
    ...
    // Generate unique identifiers for new patient
    ...
    if (newPatient != null) {
        ...
        if (identificationNumberExists(newPatient.getIdentificationNo())) {
            ui.showMessage("Error: Identification number already exists in system");
            ui.pressEnterToContinue();
            rollbackCounters(); // Roll back the counters since we're not adding this patient
            return;
        }
...
/**
 * Checks if an identification number already exists in the system
 * @param icNumber The identification number to check
 * @return true if the ID exists, false otherwise
 */
private boolean identificationNumberExists(String icNumber) {
    ...
    while (!allPatients.isEmpty()) {
        ...
        if (p.getIdentificationNo().equals(icNumber)) {
            exists = true;
            break;
        }
    }
...
public void processNextPatient() {
    ...
    if (!patientQueue.isEmpty()) {
        System.out.println("\nUP NEXT:");
        int showCount = Math.min(3, patientQueue.size());
        LinkedQueue<Patient> temp = new LinkedQueue<>();
        
        for (int i = 0; i < showCount; i++) {
            Patient next = patientQueue.dequeue();
            System.out.println((i+1) + ". " + next.getQueueID() + " - " + next.getName());
            temp.enqueue(next);
        }
        
        // Restore the next patients to queue
        while (!temp.isEmpty()) {
            patientQueue.enqueue(temp.dequeue());
        }
    }
...
public void processNextPatient() {
    ...
    if (doctor != null) {
        // Entity only stores the ID (ECB compliant)
        patient.setAssignedDoctorID(doctor.getDoctorID());
        
        // Display processing information
        System.out.println("\n NOW SERVING");
        System.out.println("=============");
        System.out.println("Queue ID: " + patient.getQueueID());
        System.out.println("Name: " + patient.getName());
        System.out.println("Assigned Doctor: " + doctor.getName());
        System.out.println("=============");
    } else {
        System.out.println("\nNo available doctors. Patient remains in queue.");
        patientQueue.enqueue(patient); // Requeue if no doctor available
    }

    // Show next patient status if any
    if (!patientQueue.isEmpty()) {
...
public void generatePatientHealthReport() {
    ...
    if (highRiskPatients > 0) {
        System.out.printf("! %d obese patients requiring priority nutrition counseling\n", highRiskPatients);
    }
    if (bmiCounts[0] > 0) {
        System.out.printf("! %d underweight patients needing dietary assessment\n", bmiCounts[0]);
    }
...
/**
 * Sets the patient counter to a specific value.
 * Used when initializing sample data to maintain correct ID sequencing.
 * 
 * @param count The value to set the patient counter to
 */
...
/**
 * Sets the queue counter to a specific value.
 * Used when initializing sample data to maintain correct ID sequencing.
 * 
 * @param count The value to set the queue counter to
 */
public void setQueueCounter(int count) {
